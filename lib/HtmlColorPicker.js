...
class HtmlColorPicker extends require('./HtmlWidget') {
    ...
    properties() {
        ...
        this.dependencies = {
            ...
            VSlider:require('./HtmlCanvas').extend({
    ...
    updateColor(){
        ...
        var style = slfocus.domNode.style
        var px = parseInt(this.sat * slrectNode.offsetWidth)
        var py = parseInt(this.lgt * slrectNode.offsetHeight)
        style.left = (px+slrectNode.offsetLeft-(slfocusNode.offsetWidth>>1))+'px'        
        style.top = (py+slrectNode.offsetTop-(slfocusNode.offsetHeight>>1))+'px'
    }
