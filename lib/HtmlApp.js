...
class HtmlApp extends HtmlWidget {
    ...
    constructor(domNode) {
        ...
        mouseNode.addEventListener('mousedown',e=>{
            ...
            if(n && n.widget){ 
                ...
                if(modal){ // filter mousedowns on modal stack
    ...
    // add a node to the rebuild list, and do a request animation frame
    _addRebuild(node){
        ...
        this._raf = window.requestAnimationFrame(_=>{
            ...
            for(var i = 0; i < rebuilds.length; i++){
                ...
                // remove our domnode but keep track of the index it was removed from
                var oldIndex = 0, child = nest.domNode;
                while( (child = child.previousSibling) != null ) oldIndex++;
                ...
                var newIndex = 0, child = node.view.domNode;
                while( (child = child.previousSibling) != null ) newIndex++;

                // move it back to where it came from
                if(oldIndex < newIndex){
                    console.log(node.view.domNode.parentNode)
                    node.view.domNode.parentNode.insertBefore(
                        node.view.domNode,
                        node.view.domNode.parentNode.children[oldIndex]
                    )
                }
            }
        })
    ...
    // entrypoint for rebuilding a node
    _rebuild(node){
        var asyncShow = []
