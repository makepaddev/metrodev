...
(function bootLoader(api) {
    ...
    function watchFileChange(){
        ...
        req.open("GET", "/reloader?"+(''+Math.random()).slice(2))
        req.send()
    }
    ...
    function loadJS(absPath, parentPath) {
        ...
        pendingLoads[absPath] = loadFileWrapper(absPath).then(function(source) {
            ...
            code.replace(/require\s*\(\s*['"](.*)['"]\s*\)/g, function(m, path){
                ...
                var prom = loadJS(subPath, absPath)
                deps.push(prom)
                prom.path = subPath
            });
            ...
            console.error("Error loading "+absPath+" from "+parentPath)
        });
